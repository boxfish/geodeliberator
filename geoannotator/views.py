...
def api_annotations(request):
    ...
    if len(startDate) > 0:
        annotations = annotations.filter(updated_at__gte=parser.parse(startDate))
    if len(endDate) > 0:
        annotations = annotations.exclude(updated_at__gte=parser.parse(endDate))
...
def api_timeline(request):
    ...
    response["earliestDate"] = Annotation.objects.filter(forum=forumId).order_by('updated_at')[0].updated_at.ctime()
    response["latestDate"] = Annotation.objects.filter(forum=forumId).order_by('-updated_at')[0].updated_at.ctime()
    response["timeline"] = []
    
    # Postgres specific
    byunit_select = {unit: "DATE_TRUNC('" + unit + "', updated_at)"} 
